...
sudo docker build --no-cache --build-arg "vg_git_revision=${vg_git_revision}" -t "${image_tag_prefix}-build" - < Dockerfile.build
sudo docker run -t "${image_tag_prefix}-build" vg version # sanity check
...
# sudo docker run -t "${image_tag_prefix}-build" make test
...
mkdir -p ctx/vg/
temp_container_id=$(sudo docker create "${image_tag_prefix}-build")
...
sudo docker build --no-cache -t "${image_tag_prefix}-run-preprecursor" ctx/
# - flatten the image, to further reduce its deploy size, and set up the runtime ENV/WORKDIR etc.
temp_container_id=$(sudo docker create "${image_tag_prefix}-run-preprecursor")
sudo docker export "$temp_container_id" | sudo docker import - "${image_tag_prefix}-run-precursor"
echo "FROM ${image_tag_prefix}-run-precursor" '
ENV PATH /vg/bin:$PATH
WORKDIR /vg' | sudo docker build -t "${image_tag_prefix}-run" -
# sanity check
sudo docker run -t "${image_tag_prefix}-run" vg version
...
# push images
sudo docker push "${image_tag_prefix}-build"
sudo docker push "${image_tag_prefix}-run"
